        ...
        const contactoSchema = new mongoose.Schema({
            ...
            // Esquema para la información de la compra
...
//         });
        ...
        app.get('/pelicula-comprada/:iduser', async (req, res) => {
            ...
            try {
                const compras = await Compra.find({ iduser: iduser }).populate('idmovie', 'nombre foto'); 
                if (!compras || compras.length === 0) {
                    return res.status(404).json({ message: 'Usuario no encontrado o no tiene compras' });
                }
        ...
        app.get('/buscar-movie', async (req, res) => {
            ...
            if (!nombre) {
            return res.status(400).json({ message: 'El nombre de la película es obligatorio' });
            }
        
            try {
            const movie = await Movie.findOne({ nombre: new RegExp(nombre, 'i') }).populate('idgenero').populate('idclasificacion');
        ...
        app.post("/contacto", async (req, res) => {
            try {
              ...
              const nuevoContacto = new Contacto({ nombre, email, comentarios });
